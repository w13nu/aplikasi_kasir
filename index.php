...
<div class="col-lg-9" id="print">
    <form id="myCartNew" method="post">
            ...
            <div class="col-12 col-lg-3 m-pr-0">
                ...
                <div class="input-group">
                    <input type="text" class="form-control form-control-sm border-right-0" list="datalist1"
                        id="kode_produk" onkeyup="filterNamaProduk(this.value)" aria-describedby="basic-addon2" required>
                    <datalist id="datalist1">
                        <?php  
                        if (mysqli_num_rows($barang)) {
                            while ($row_brg = mysqli_fetch_array($barang)) { ?>
                                <option value="<?php echo $row_brg["kode_produk"] ?>">
                                    <?php echo $row_brg["kode_produk"] ?>
                                </option>
                                <?php 
                                $jsArray .= "harga_jual['" . $row_brg['kode_produk'] . "'] = '" . addslashes($row_brg['harga_jual']) . "';";
                                $jsArray1 .= "nama_produk['" . $row_brg['kode_produk'] . "'] = '" . addslashes($row_brg['nama_produk']) . "';";
                                $jsArray3 .= "idproduk['" . $row_brg['kode_produk'] . "'] = '" . addslashes($row_brg['idproduk']) . "';";
                                $jsArray4 .= "stock['" . $row_brg['kode_produk'] . "'] = '" . addslashes($row_brg['stock']) . "';";
                            }
                        } ?>
                    </datalist>
        ...
        <?php 
                    $barang = mysqli_query($conn, "SELECT * FROM produk ORDER BY idproduk ASC");
                    $jsArray2 = "var kode_produk = new Array();";
                ?>
            ...
            <input type="text" class="form-control form-control-sm border-right-0" list="datalistNamaProduk"
                    id="nama_produk" onkeyup="filterKodeProduk(this.value)" aria-describedby="basic-addon2" required>
                <datalist id="datalistNamaProduk">
                    <?php  
                    if (mysqli_num_rows($barang)) {
                        while ($row_brg = mysqli_fetch_array($barang)) { ?>
                            <option value="<?php echo $row_brg["nama_produk"] ?>">
                                <?php echo $row_brg["nama_produk"] ?>
                            </option>
                            <?php 
                            $jsArray .= "harga_jual['" . $row_brg['nama_produk'] . "'] = '" . addslashes($row_brg['harga_jual']) . "';";
                            $jsArray2 .= "kode_produk['" . $row_brg['nama_produk'] . "'] = '" . addslashes($row_brg['kode_produk']) . "';";
                            $jsArray3 .= "idproduk['" . $row_brg['nama_produk'] . "'] = '" . addslashes($row_brg['idproduk']) . "';";
                            $jsArray4 .= "stock['" . $row_brg['nama_produk'] . "'] = '" . addslashes($row_brg['stock']) . "';";
                        }
                    } ?>
                </datalist>
        </div>
...
<script>
    ...
    function filterNamaProduk(value) {
        document.getElementById("nama_produk").value = nama_produk[value] ? nama_produk[value] : "";
        document.getElementById("idproduk").value = idproduk[value] ? idproduk[value] : "";
        document.getElementById("harga_jual").value = harga_jual[value] ? harga_jual[value] : "";
        document.getElementById("stock").value = stock[value] ? stock[value] : "";
    }

    function filterKodeProduk(value) {
        document.getElementById("kode_produk").value = kode_produk[value] || '';
        document.getElementById("idproduk").value = idproduk[value] || '';
        document.getElementById("harga_jual").value = harga_jual[value] || '';
        document.getElementById("stock").value = stock[value] || '';
    }
    ...
    function total() {
     ...
     // Menambahkan event listener untuk kode produk agar pemindaian barcode memicu filter
     document.getElementById("kode_produk").addEventListener('change', function() {
        filterNamaProduk(this.value);
    });
</script>
<?php
...
<div class="d-none pt-5 px-4 print-show">
    <div class="row">
        ...
        <div class="col-12">
            ...
            <h3>Metode <span class="float-right" id="metode-print">Cash</span></h3>
            <h3>Bayar <div class="float-right">Rp.<span id="bayarnya1"></span></div></h3>
        ...
        <div class="col-12 text-center">
            ...
            <p class="h4 text-muted">Powered One.Click</p>
        </div>
    </div><!-- end row -->
</div><!-- end box print -->
                        ...
                        <div class="col-lg-5 mb-2 print-none">
                            ...
                            <div class="col-5 mb-2 text-right pt-1 pr-2" style="font-weight:500;">Pembayaran :</div>
                                <div class="col-7 mb-2 pl-0">
                                    ...
                                    <select class="form-control form-control-sm mb-2" name="metode" id="metode" required>
...
if(isset($_POST['save'])){
    ...
    SELECT no_nota,idproduk,quantity FROM keranjang") 
     ...
     $insertlaporan = mysqli_query($conn, "INSERT INTO laporan (no_nota,idpelanggan,catatan,pembayaran,kembalian,totalbeli,metode)
    VALUES ('$nonota','$idpell','$catatan','$pembayaran','$kembalian','$totalbeli','$metode')");
...
function total() {
  ...
  function changeValuePelanggan(nama_pelanggan) {
    ...
    document.querySelector('input[list="datalist2"]').value = "UMUM";
    changeValuePelanggan("UMUM");
  });
</script>
